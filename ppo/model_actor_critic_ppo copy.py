...
class Model:
    ...
    def _update_ppo(self) -> Dict[str, float]:
        ...
        def _finite(name, t):
            ...
            if not ok:
                print(f"[WARN] non-finite in {name}, skipping PPO update")
            return ok

        if not (_finite("b_states", b_states)
                and _finite("b_actions", b_actions)
                and _finite("b_log_probs", b_log_probs)
                and _finite("b_td_targets", b_td_targets)
                and _finite("advantages", advantages)):
            # пропускаем всю PPO-эпоху, но НЕ трогаем веса
            return {
                "loss": float("nan"),
                "value_loss": float("nan"),
                "grad_norm": float("nan"),
        ...
        for i in range(self.cfg.ppo_epochs):
            ...
            for start in range(0, batch_size, mbs):
                ...
                if not valid_gradients:
                    print("[WARN] NaN/Inf in gradients, skip optimizer.step()")
                    self.optimizer.zero_grad(set_to_none=True)
                    continue
